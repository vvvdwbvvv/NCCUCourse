    ...
    try:
        ...
        if isOld:
            ...
            for objective in contents[0].find("div", {"class": "qa_content"}):
                for line in [x for x in re.split(r'[\n\r]+', objective.get_text(strip=True)) if
                             len(x) > 0 and x != " "]:
            ...
            description_title = soap.find("div", {"class": "col-sm-7 sylview--mtop col-p-6"}).find("h2", {
                "class": "text-primary"})
            descriptions = description_title.find_next_siblings(True)
            for description in descriptions:
                if description.attrs and description.attrs.get("class") and ["row", "sylview-mtop", "fa-border"] == \
                        description.attrs["class"]:
                    break
                for line in [x for x in re.split(r'[\n\r]+', description.get_text(strip=True)) if
                             len(x) > 0 and x != " "]:
                    result["description"].append(line)
